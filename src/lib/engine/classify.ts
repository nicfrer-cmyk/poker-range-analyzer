...
function classifyMadeTier(combo: Combo, board: Card[]): MadeTier {
  ...
  switch (evaluated.category) {
    ...
    case 'pair': {
      ...
      if (matchedRank === undefined) return 'air'; // shouldn't happen if evaluated.category === 'pair'
...
function findStraightDraws(uniqueRankValues: Set<number>): DrawType[] {
  ...
  for (let windowMin = -1; windowMin <= 8; windowMin++) {
    ...
    if (present.length === 4) {
      ...
      if (missing === windowMin || missing === windowMin + 4) {
        sawOpenEnded = true;
      } else {
        sawGutshot = true;
      }
  ...
  if (sawOpenEnded) draws.push('open-ended-straight-draw');
  else if (sawGutshot) draws.push('gutshot-straight-draw');
  return draws;
}
